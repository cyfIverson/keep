...
public class YieldMethod extends Thread{
    ...
    public static void main(String[] args) {
        YieldMethod yieldMethod1 = new YieldMethod("张三");
        YieldMethod yieldMethod2 = new YieldMethod("李四");

        yieldMethod1.start();
        yieldMethod2.start();

        /*
        * 运行结果：第一种情况：李四（线程）当执行到30时会CPU时间让掉，这时张三（线程）抢到CPU时间并执行。
